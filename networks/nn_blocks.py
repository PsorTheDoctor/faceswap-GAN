...
def dual_attn_block(inp, nc, squeeze_factor=8):
    ...
    x_pam = Conv2D(nc, kernel_size=3, kernel_regularizer='l2',  
               kernel_initializer=conv_init, use_bias=False, padding="same")(x)
    x_pam = Activation("relu")(x_pam)
    x_pam = normalization(x_pam, norm, nc)
    f_pam = Conv2D(nc//squeeze_factor, 1, kernel_regularizer='l2')(x_pam)
    g_pam = Conv2D(nc//squeeze_factor, 1, kernel_regularizer='l2')(x_pam)
    h_pam = Conv2D(nc, 1, kernel_regularizer='l2')(x_pam)    
    ...
    out_pam = Conv2D(nc, kernel_size=3, kernel_regularizer='l2',  
    ...
    x_chn = Conv2D(nc, kernel_size=3, kernel_regularizer='l2',  
               kernel_initializer=conv_init, use_bias=False, padding="same")(x)
    ...
    out_chn = Conv2D(nc, kernel_size=3, kernel_regularizer='l2',  
...
def normalization(inp, norm='none', group='16'):    
    ...
    if norm == 'layernorm':
        ...
        x0 = Conv2D(f//2, kernel_size=1, kernel_regularizer='l2',
...
def conv_block(input_tensor, f, use_norm=False, strides=2, w_l2=w_l2, norm='none'):
    x = input_tensor
    x = Conv2D(f, kernel_size=3, strides=strides, kernel_regularizer='l2',  
               kernel_initializer=conv_init, use_bias=False, padding="same")(x)
    x = Activation("relu")(x)
...
def conv_block_d(input_tensor, f, use_norm=False, w_l2=w_l2, norm='none'):
    x = input_tensor
    x = Conv2D(f, kernel_size=4, strides=2, kernel_regularizer='l2', 
               kernel_initializer=conv_init, use_bias=False, padding="same")(x)
...
def res_block(input_tensor, f, use_norm=False, w_l2=w_l2, norm='none'):
    x = input_tensor
    x = Conv2D(f, kernel_size=3, kernel_regularizer='l2', 
               kernel_initializer=conv_init, use_bias=False, padding="same")(x)
    x = LeakyReLU(alpha=0.2)(x)
    x = normalization(x, norm, f) if use_norm else x
    x = Conv2D(f, kernel_size=3, kernel_regularizer='l2', 
...
def SPADE_res_block(input_tensor, cond_input_tensor, f, use_norm=True, norm='none'):
    ...
    def SPADE(input_tensor, cond_input_tensor, f, use_norm=True, norm='none'):
        ...
        x = normalization(x, norm, f) if use_norm else x
        y = cond_input_tensor
        y = Conv2D(128, kernel_size=3, kernel_regularizer='l2', 
                   kernel_initializer=conv_init, padding='same')(y)
        y = Activation('relu')(y)           
        gamma = Conv2D(f, kernel_size=3, kernel_regularizer='l2', 
                   kernel_initializer=conv_init, padding='same')(y)
        beta = Conv2D(f, kernel_size=3, kernel_regularizer='l2', 
                   kernel_initializer=conv_init, padding='same')(y)
...
def upscale_ps(input_tensor, f, use_norm=False, w_l2=w_l2, norm='none'):
    x = input_tensor
    x = Conv2D(f*4, kernel_size=3, kernel_regularizer='l2', 
